...
%TCIDATA{LastRevised=Tuesday, August 11, 2015 17:58:27}
...
\graphicspath{{C:/Users/Silvana/Dropbox/impvol/Finalversion/impvol_text/}}
\input{tcilatex}
\begin{document}
...
\begin{center}
\begin{figure}[h]
\centering\includegraphics[width=0.8\textwidth]{fig1}
...
%% in fact, i would put this in utility to ease on terminology. 
%% than J could be called final goods
%% omega(j) would be intermediates
...
\label{QJ}
\end{equation}%
%% importantly, this remains the same in the open economy
...
\right] ,$ and $\Gamma $ is the gamma function. Using (\ref{QJ}), (\ref%
{Labor share}), and (\ref{Pt}) we obtain real GDP: 
XX FIXME: Silvana, how do you derive equ (12) in the original manuscript?
\begin{equation}
Y_{t}=\sum_{j=1}^{J}\frac{w_{t}^{j}L_{t}^{j}}{P_{t}}=\prod_{j=1}^{J}R_{j}%
\left[ A_{t}^{j}\right] ^{\frac{\alpha ^{j}}{\bar{\beta}}}\left(
L_{t}^{j}\right) ^{\frac{\alpha ^{j}\beta ^{j}}{\bar{\beta}}},  \label{gdp}
\end{equation}%
where $\bar{\beta}=\sum_{j=1}^{J}\alpha ^{j}\beta ^{j}$ and $R_{j}\propto
\prod_{j=1}^{J}\left( \beta ^{j}\alpha ^{j}\right) ^{-\frac{\alpha ^{j}\beta
^{j}}{\bar{\beta}}}(B^{j})^{-\frac{\alpha ^{j}}{\bar{\beta}}}\left(
T^{j}\right) ^{\frac{\alpha ^{j}}{\bar{\beta}\cdot \theta }}$ is a
time-invariant product.

We can now move one step backward and solve for the allocation of the
primary input across sectors, $L_{t}^{j},$ $j=1,...,J$. From (\ref%
{eq:log:utility}) and (\ref{eq:FOC}), $L_{t}^{j}=\frac{\alpha ^{j}\beta ^{j}%
}{\bar{\beta}}L_{t}$.\footnote{%
The FOC are $\frac{\alpha ^{j}\beta ^{j}}{\bar{\beta}}E[u^{%
\prime}(C_{t})C_{t}]=\upsilon L_{t}^{j}$ where $\upsilon $ is the multiplier
for the resource constraint.} Substituting into (\ref{gdp}), GDP is given
by: 
XX FIXME
\begin{equation}
Y_{t}=\prod_{j=1}^{J}R_{j}\left( \alpha ^{j}\frac{\beta ^{j}}{\bar{\beta}}%
\right) ^{\frac{\alpha ^{j}\beta ^{j}}{\bar{\beta}}}\left( A_{t}^{j}\right)^{%
\frac{\alpha ^{j}}{\bar{\beta}}}L_{t}  \label{expost}
\end{equation}
In words, GDP varies with fluctuations in sectoral productivity, $A_{t}^{j}$%
, and aggregate resources, $L_{t}.$
...
\begin{equation*}
p_{nmt}^{j}(\omega ^{j})=\frac{B^{j}
	\left( w_{mt}^{j}\right) ^{\beta^{j}}
	\prod_{k=1}^J (P_{t}^k)^{\gamma^{kj}}
}
	{A_{mt}^{j}\kappa _{nmt}^{j}z_{m}(\omega ^{j})}
\end{equation*}%
where $B^{j}\left( w_{mt}^{j}\right) ^{\beta ^{j}}\prod_{k=1}^J (P_{t}^k)^{\gamma^{kj}}$ is
...
(P_{nt}^{j}<p)=1-\exp \left[ -\Phi _{nt}^{j}p^{\theta }\right] $ 
where 
$\Phi_{nt}^{j}=
	\sum_{m=1}^{N}
	T_{m}^{j}
	\left( 
		\frac
			{B^{j}\left( w_{mt}^{j}\right)^{\beta ^{j}}\prod_{k=1}^J (P_{t}^k)^{\gamma^{kj}}}
			{A_{mt}^{j}\kappa _{nmt}^{j}}
	\right)^{-\theta }
$. Given that there is a continuum of $\omega ^{j}$ in each
...
obtain: 
\begin{equation}
d_{nm}^{j} = \frac 1 {(\xi B^j)^\theta}
	(\kappa_{nm}^j)^\theta
	(P_{n}^j)^\theta Z_m^j
	(\psi_{m}^j)^{\theta\beta^j}
	(y_m^j)^{-\theta\beta^j}
	(P_m)^{-\theta\beta^j}
	\prod_{k=1}^J
		(P_{m}^k)^{-\theta\gamma^{kj}},
  \label{produc}
\end{equation}%
...
Z_{mt}^{j}
=
{B^{j}}^{\theta }{\xi }^{\theta }d_{nmt}^{j}\left(
y_{m}^{j}\right) ^{\theta \beta ^{j}}\left( \kappa _{nmt}^{j}\right)
^{-\theta }\left( \frac{P_{nt}^{j}}
	{P_{mt}^{\beta^j}
	\prod_{k=1}^J
		(P_{mt}^k)^{\gamma^{kj}}}
	\right) ^{-\theta }\left( \psi
...
XX WORKING ON EQ BELOW
\begin{align}
Z_{nt}^{j}& =\underbrace
	{
		{B^{j}}^{\theta }{\xi }^{\theta }d_{knt}^{j}\left(
		y_{n}^{j}\right) ^{\theta \beta ^{j}}\left( \kappa _{knt}^{j}\right)
		^{-\theta }\left( \frac{P_{kt}^{j}}
			{P_{nt}^{\beta^j}
			\prod_{m=1}^J
				(P_{nt}^m)^{\gamma^{mj}}}
			\right) ^{-\theta }\left( \psi_{nt}^{j}\right) ^{-\theta \beta ^{j}}	
	}_{\equiv \exp (\zeta _{k,n,t}^{j})}{%
P_{k,t}^{j}}^{-\theta }  \notag \\
...
\centering\includegraphics[width=\textwidth]{tab3.pdf}
\end{table}
...
\theta =8$.}\centering\includegraphics[width=\textwidth]{tab4.pdf}
\end{table}
