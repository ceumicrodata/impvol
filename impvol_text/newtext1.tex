...
^{-\theta }\left( \frac{P_{kt}^{j}}{P_{nt}^{\beta
^{j}}\prod_{m=1}^{J}(P_{nt}^{m})^{\gamma ^{mj}}}\right) ^{-\theta }\left(
\psi _{nt}^{j}\right) ^{-\theta \beta ^{j}}}_{\equiv \exp (\zeta
...
differential between the average equipped labour cost in the economy $\frac{1%
}{J}\sum_{j=1}^{J}w_{nt}^{j}$ and the sectoral input cost $w_{nt}^{k}$.
...
\end{table}
We compute labour shares and sectoral wage differentials $\left( \frac{1}{J}%
\sum_{j=1}^{J}w_{nt}^{j}-w_{nt}^{k}/%
